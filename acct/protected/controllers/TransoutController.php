...
class TransoutController extends Controller 
{
	...
	{
		$model = new TransOutList;
		if (isset($_POST['TransOutList'])) {
	...
	public function actionSave()
	{
		if (isset($_POST['TransOutForm'])) {
			$model = new TransOutForm($_POST['TransOutForm']['scenario']);
	...
	{
		$model = new TransOutForm('new');
		$this->render('form',array('model'=>$model,));
	}
	
	public function actionEdit($index)
	{
		$model = new TransOutForm('edit');
		if (!$model->retrieveData($index)) {
			throw new CHttpException(404,'The requested page does not exist.');
		} else {
			$this->render('form',array('model'=>$model,));
	...
	public static function allowReadWrite() {
		return Yii::app()->user->validRWFunction('XE03');
	}
	
	public static function allowReadOnly() {
		return Yii::app()->user->validFunction('XE03');
	}
}
