...
class RemitAuditList extends CListPageModel
{
	/**
	...
	public function retrieveDataByPage($pageNum=1)
	{
		...
		$sql1 = "select a.*,g.name as city_name,f.name as department_name,b.code as employee_code,b.name as employee_name 
				...
				where a.status_type in (2,4,6) and a.table_type=2 and FIND_IN_SET('{$uid}',a.audit_user)
			";
		...
		$session['remitAudit_c01'] = $this->getCriteria();
		return true;
	}
    ...
    public function getCountConsult(){
        ...
        $sql = "select count(id)
				from acc_expense 
				where status_type=2 and table_type=2 and current_username = '{$uid}'
			";
