...
class PlaneSetYearForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $set_name;
	...
	public function attributeLabels(){
		return array(
            ...
            'value_name'=>Yii::t('plane','Set Year'),
	...
	public function retrieveData($index){
		...
		$sql = "select * from acc_plane_set_year where id='".$index."' and city='{$city}'";
		...
		if ($row!==false) {
            ...
            $rows = Yii::app()->db->createCommand()->select("id,value_name,value_money")->from("acc_plane_set_year_info")
                ->where("year_id=:id",array(":id"=>$index))->order("value_name asc")->queryAll();
            if($rows){
                foreach ($rows as $arr){
                    $temp = array();
                    $temp['id'] = $arr['id'];
	...
	protected function saveInfo(&$connection){
        ...
        foreach ($this->info_list as $row) {
            ...
            switch ($this->scenario) {
                case 'delete':
                    $sql = "delete from acc_plane_set_year_info where year_id = :year_id";
                    break;
                case 'new':
                    if ($row['uflag']=='Y') {
                        $sql = "insert into acc_plane_set_year_info(
									year_id, value_name, value_money
								) values (
									:year_id, :value_name, :value_money
								)";
                    }
                ...
                case 'edit':
                    switch ($row['uflag']) {
                        case 'D':
                            $sql = "delete from acc_plane_set_year_info where id = :id";
                            break;
                        case 'Y':
                            $sql = ($row['id']==0)
                                ?
                                "insert into acc_plane_set_year_info(
										year_id, value_name, value_money
									) values (
										:year_id, :value_name, :value_money
									)"
                                :
                                "update acc_plane_set_year_info set
...
//                print_r($sql);exit();
                ...
                if (strpos($sql,':year_id')!==false)
                    $command->bindParam(':year_id',$this->id,PDO::PARAM_INT);
	...
	protected function saveDataForSql(&$connection){
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from acc_plane_set_year where id = :id";
				break;
			case 'new':
				$sql = "insert into acc_plane_set_year(
						set_name,start_date, city, lcu, lcd) values (
						:set_name, :start_date, :city, :lcu, :lcd)";
				break;
			case 'edit':
				$sql = "update acc_plane_set_year set 
					set_name = :set_name, 
