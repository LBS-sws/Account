...
class TemporaryPaymentList extends CListPageModel
{
	/**
	...
	public function retrieveDataByPage($pageNum=1)
	{
		...
		$sql1 = "select a.*,g.name as city_name,f.name as department_name,b.code as employee_code,b.name as employee_name 
				...
				where a.status_type in (4,6) and a.table_type=3 and a.city = '{$city}' 
			";
		...
		if (count($records) > 0) {
			foreach ($records as $k=>$record) {
                $this->attr[] = array(
                    ...
                    'color'=>$record['status_type']==4?"bg-yellow":"",
                    'status_str'=>ExpenseFun::getStatusStrForStatusType($record['status_type']),
                );
			}
		}
		$session = Yii::app()->session;
		$session['temporaryPayment_c01'] = $this->getCriteria();
		return true;
	}
    ...
    public function getCountConsult(){
        ...
        $sql = "select count(id)
				from acc_expense 
				where status_type=4 and table_type=3 and city='{$city}'
			";
