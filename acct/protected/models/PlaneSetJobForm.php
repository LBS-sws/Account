...
class PlaneSetJobForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $set_name;
	...
	public function attributeLabels(){
		return array(
            ...
            'value_name'=>Yii::t('plane','Set Job'),
	...
	public function retrieveData($index){
		...
		$sql = "select * from acc_plane_set_job where id='".$index."' and city='{$city}'";
		...
		if ($row!==false) {
            ...
            $rows = Yii::app()->db->createCommand()->select("id,value_name,value_money")->from("acc_plane_set_job_info")
                ->where("job_id=:id",array(":id"=>$index))->order("value_money asc")->queryAll();
            if($rows){
                foreach ($rows as $arr){
                    $temp = array();
                    $temp['id'] = $arr['id'];
    ...
    public static function getPlaneList($date){
        ...
        if($row){
            ...
            if($arr){
                foreach ($arr as $item){
                    $list[$item["id"]] = array("name"=>$item["value_name"]." - {$item["value_money"]}","num"=>$item["value_money"],"id"=>$item["id"]);
                }
            }
        }
        return $list;
    ...
    public static function getPlaneName($id){
        $value_name = Yii::app()->db->createCommand()->select("value_name")->from("acc_plane_set_job_info")
            ->where("id=:id",array(":id"=>$id))->queryScalar();
        return $value_name?$value_name:$id;
    }
	...
	protected function saveInfo(&$connection){
        ...
        foreach ($this->info_list as $row) {
            ...
            switch ($this->scenario) {
                case 'delete':
                    $sql = "delete from acc_plane_set_job_info where job_id = :job_id";
                    break;
                case 'new':
                    if ($row['uflag']=='Y') {
                        $sql = "insert into acc_plane_set_job_info(
									job_id, value_name, value_money
								) values (
									:job_id, :value_name, :value_money
								)";
                    }
                ...
                case 'edit':
                    switch ($row['uflag']) {
                        case 'D':
                            $sql = "delete from acc_plane_set_job_info where id = :id";
                            break;
                        case 'Y':
                            $sql = ($row['id']==0)
                                ?
                                "insert into acc_plane_set_job_info(
										job_id, value_name, value_money
									) values (
										:job_id, :value_name, :value_money
									)"
                                :
                                "update acc_plane_set_job_info set
...
//                print_r($sql);exit();
                ...
                if (strpos($sql,':job_id')!==false)
                    $command->bindParam(':job_id',$this->id,PDO::PARAM_INT);
	...
	protected function saveDataForSql(&$connection){
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from acc_plane_set_job where id = :id";
				break;
			case 'new':
				$sql = "insert into acc_plane_set_job(
						set_name,start_date, city, lcu, lcd) values (
						:set_name, :start_date, :city, :lcu, :lcd)";
				break;
			case 'edit':
				$sql = "update acc_plane_set_job set 
					set_name = :set_name, 
