...
//        print_r($month);
        ...
        $sql1 = "select a.*,c.name,d.new_amount,d.edit_amount,d.end_amount,d.performance_amount,d.performanceedit_amount,d.performanceend_amount,d.renewal_amount,d.renewalend_amount,d.product_amount,e.name as cityname from acc_service_comm_hdr a
			     ...
			     where  a.year_no='$year' and a.month_no='$month' and a.city='".$city."' and b.city='$city' and (b.staff_status!='-1' or (b.staff_status='-1' and replace(b.leave_time,'-', '/')>='$leaveTime'))
			";
...
//        print_r($this);
    ...
    public function renewalendDataByPage($pageNum=1,$year,$month,$index)
    {
        ...
        $sql1="select a.*,  c.description as type_desc, d.name as city_name					
				...
				inner join security$suffix.sec_city d on a.city=d.code 			  
				left outer join swoper$suffix.swo_customer_type c on a.cust_type=c.id 
				where a.city ='$city'   and a.status='T' and a.status_dt>='$start' and a.status_dt<='$end'  and  a.salesman ='".$name['name']."' and surplus!=0 and surplus_edit0!=0 and surplus_edit1!=0 and surplus_edit2!=0 and surplus_edit3!=0 
";
        $clause = "";
        if (!empty($this->searchField) && !empty($this->searchValue)) {
            $svalue = str_replace("'","\'",$this->searchValue);
...
//        print_r($years);  exit();
    ...
    public  function getProductctAmount($city, $cust_type,$sales_products, $start_dt, $sales_amt) {
        //城市，类别，时间，总金额
        $rtn = 0;
        if (!empty($city) && !empty($cust_type) && !empty($start_dt) && !empty($sales_amt)) {//!empty($sales_amt)
            $suffix = Yii::app()->params['envSuffix'];
            //客户类别
