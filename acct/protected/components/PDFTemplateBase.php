...
class PDFTemplateBase extends TCPDF {
	...
	protected function getLabelHeightEx() {
		...
		foreach ($this->hdr_def as $def) {
			if (is_array($def)) {
				$h = $this->calcHeaderBlockHeight($def, $this->line_def);
			} else {
	...
	protected function calcHeaderBlockHeight($item) {
		...
		$w = $this->calcHeaderBlockWidth($item, $this->line_def);
	...
	protected function calcHeaderBlockWidth($item) {
		...
		foreach ($item['child'] as $child) {
			if (is_array($child)) {
				$rtn += $this->calcHeaderBlockWidth($child, $this->line_def);
			} else {
