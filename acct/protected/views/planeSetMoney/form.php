<?php
$this->pageTitle=Yii::app()->name . ' - PlaneSetMoney Form';
?>

<?php $form=$this->beginWidget('TbActiveForm', array(
'id'=>'PlaneSetMoney-form',
...
<section class="content-header">
	<h1>
		<strong><?php echo Yii::t('plane','Plane Set Money Form'); ?></strong>
	</h1>
<!--
	<ol class="breadcrumb">
		...
		<?php 
			if ($model->scenario!='new' && $model->scenario!='view') {
				echo TbHtml::button('<span class="fa fa-file-o"></span> '.Yii::t('misc','Add Another'), array(
					'submit'=>Yii::app()->createUrl('planeSetMoney/new')));
			}
		?>
		<?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
				'submit'=>Yii::app()->createUrl('planeSetMoney/index')));
		?>
<?php if ($model->scenario!='view'): ?>
			<?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('misc','Save'), array(
				'submit'=>Yii::app()->createUrl('planeSetMoney/save')));
			?>
<?php endif ?>
<?php if ($model->scenario!='new' && $model->scenario!='view'): ?>
            <?php echo TbHtml::button('<span class="fa fa-clone"></span> '.Yii::t('misc','Copy'), array(
                    'submit'=>Yii::app()->createUrl('planeSetMoney/new', array('index'=>$model->id)))
            );
            ?>
                ...
                <div class="box-body table-responsive">
                    <div class="col-lg-6 col-lg-offset-3">
                        ...
                        $this->widget('ext.layout.TableView2Widget', array(
                            'model'=>$model,
                            'attribute'=>'info_list',
                            'viewhdr'=>'//planeSetMoney/_formhdr',
                            'viewdtl'=>'//planeSetMoney/_formdtl',
                        ));
                        ?>
                    </div>
                    ...
                    <div class="col-lg-3" style="border: 1px solid #000;">
                        ...
                        <div class="form-group">
                            <?php echo $form->labelEx($model,'做单递增',array('class'=>"col-lg-5 control-label")); ?>
                            <div class="col-lg-5">
                                <?php echo TbHtml::numberField("addValue", 500,
                                    array('id'=>'addValue')
                                ); ?>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-5">
                                <?php echo TbHtml::numberField("addMoney", 50,
                                    array('id'=>'addMoney')
                                ); ?>
                            </div>
                        </div>
...
<?php
$js = "
$('table').on('change','[id^=\"PlaneSetMoneyForm\"]',function() {
	var n=$(this).attr('id').split('_');
	$('#PlaneSetMoneyForm_'+n[1]+'_'+n[2]+'_uflag').val('Y');
});
";
...
$('#btnAddRow').on('click',function() {
	...
	if (r>0) {
		...
		$('#tblDetail tr').eq(-1).find('[id*=\"PlaneSetMoneyForm_\"]').each(function(index) {
			var id = $(this).attr('id');
...
$js = Script::genDeleteData(Yii::app()->createUrl('planeSetMoney/delete'));
