...
<?php $form=$this->beginWidget('TbActiveForm', array(
    ...
    'action'=>Yii::app()->createUrl('IDCommission/new',array('index'=>$model->id,'type'=>$this->type,'year'=>$model->year,'month'=>$model->month)),
)); ?>
...
<section class="content-header">
	<h1>
        <strong>
            <?php
            if($this->type == 1){
                echo "ID ".Yii::t('app','Sales New Commission');
            }else{
                echo "ID ".Yii::t('app','Sales New Query');
            }
            ?>
        </strong>
	</h1>
        ...
        <div class="box-body">
            <div class="btn-group" role="group">
                <?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('misc','Save1'), array(
                        'submit'=>Yii::app()->createUrl('iDCommission/newsave',array('index'=>$model->id,'type'=>$this->type,'year'=>$model->year,'month'=>$model->month)))
                ); ?>
            </div>
        </div>
    </div>
