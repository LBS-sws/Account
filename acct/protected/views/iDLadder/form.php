<?php
$this->pageTitle=Yii::app()->name . ' - Service Rate Form';
?>
...
<section class="content-header">
	<h1>
		<strong>ID<?php echo Yii::t('service','Service Rate Form'); ?></strong>
	</h1>
</section>
...
<section class="content">
	...
	<div class="btn-group" role="group">
		<?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
				'submit'=>Yii::app()->createUrl('IDLadder/index')));
		?>
<?php if (!$model->isReadOnly()): ?>
		<?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('misc','Save'), array(
				'submit'=>Yii::app()->createUrl('IDLadder/save')));
		?>
<?php endif ?>
        <?php if ($model->scenario!="new"): ?>
            <?php echo TbHtml::button('<span class="fa fa-clone"></span> '.Yii::t('misc','Copy'), array(
                    'submit'=>Yii::app()->createUrl('IDLadder/new', array('index'=>$model->id)))
            );
            ?>
            <?php echo TbHtml::button('<span class="fa fa-remove"></span> '.Yii::t('misc','Delete'), array(
                'submit'=>Yii::app()->createUrl('IDLadder/delete')));
            ?>
        <?php endif ?>
	</div>
	</div></div>
...
-->
			<div class="form-group">
				<?php echo $form->labelEx($model,'only_num',array('class'=>"col-sm-2 control-label")); ?>
				<div class="col-sm-3">
                    <?php
                     echo $form->inlineRadioButtonList($model,'only_num',array(Yii::t("misc","No"),Yii::t("misc","Yes")),
                         array('readonly'=>$model->isReadOnly()));
                    ?>
				</div>
			</div>
...
<div class="box-body table-responsive">
			<?php 
				$this->widget('ext.layout.TableView2Widget', array(
					'model'=>$model,
					'attribute'=>'detail',
					'viewhdr'=>'//iDLadder/_formhdr',
					'viewdtl'=>'//iDLadder/_formdtl',
				));
			?>
</div>			
</div>			
...
<?php
$js = "
$('table').on('change','[id^=\"IDLadderForm\"]',function() {
	var n=$(this).attr('id').split('_');
	$('#IDLadderForm_'+n[1]+'_'+n[2]+'_uflag').val('Y');
});
";
...
$('#btnAddRow').on('click',function() {
	...
	if (r>0) {
		...
		$('#tblDetail tr').eq(-1).find('[id*=\"IDLadderForm_\"]').each(function(index) {
			var id = $(this).attr('id');
